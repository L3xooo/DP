...
def _make_env(seed: int = 0, T: int = 6, N: int = 4, F: int = 3) -> tuple[PortfolioEnv, np.ndarray, int, int]:
    rng = np.random.default_rng(seed)
    features = rng.normal(size=(T, N, F)).astype(np.float32)
    prices = rng.uniform(50, 200, size=(T, N)).astype(np.float32)
    tickers = [f"T{i}" for i in range(N)]
    env = PortfolioEnv(features=features, prices=prices, tickers=tickers, app_config=AppConfig())
...
class TestPortfolioGetPrices:
    ...
    def test_get_prices_returns_exact_values(self, current_step, extra_step, expected, should_raise):
...
class TestPortfolioValue:
    ...
    @pytest.mark.parametrize(
        "prices_arg, expected",
        [
            (np.array([100.0, 200.0], dtype=np.float32), 9100),   # 15 * 100 + 3 * 200 + 7000 = 1500 + 600 + 7000 = 9100
    ...
    def test_calculate_portfolio_value_raises_when_price_and_share_sizes_mismatch(self):
        env = PortfolioEnv.__new__(PortfolioEnv)

        env.portfolio_cash = PrevCurr(prev=0.0, curr=0.0)
        env.shares = PrevCurr(prev=np.zeros(3, dtype=np.float32), curr=np.array([1.0, 2.0, 3.0], dtype=np.float32))
...
class TestPortfolioReset:
    def test_reset_overwrites_previous_state(self):
        ...
        env.shares = PrevCurr(prev=np.ones(N, dtype=np.float32), curr=np.ones(N, dtype=np.float32) * 5)
        env.assets_prices = PrevCurr(prev=np.ones(N, dtype=np.float32), curr=np.ones(N, dtype=np.float32) * 6)
...
class TestPortfolioReward:
    ...
    def test_calculate_reward_handles_zero_portfolio_value(self, monkeypatch):
        ...
        monkeypatch.setattr(env, "_calculate_portfolio_value", lambda prices=None: 0.0 if prices is None else 1.0)
