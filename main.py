...
if __name__ == "__main__":
    ...
    for episode in range(app_config.number_of_episodes):
        ...
        while True:
            if done:
                logger.info(
                    f"Episode {episode} finished, total reward {episode_reward:.2f} portfolio value {env.portfolio_value.curr}"
                )
            ...
            action, noisy_logits = td3_agent.select_action(
                state, 0.75, None, None
            )
            ...
            c1, c2 = td3_agent.update(
                env.replay_buffer, batch_size=128, temperature=0.75
            )
